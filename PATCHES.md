...
| C | `release()` throws 90001 on a FAILED channel and leaks the state-change controller | `lib/src/impl/realtime_channels_impl.dart:201-209` awaits `channel.detach()` unguarded; `detach()` raises 90001 from FAILED at `realtime_channel_impl.dart:683-691`, so `_channels.remove()` at line 207 never runs and `dispose()` is never called | Unconditional removal from `_channels` up front; `detach()` wrapped in try/`on AblyException` (logged, swallowed); `dispose()` moved into `finally` so it runs even if `detach()` throws something else | `patch/c-release-idempotent` | `681211f2eca7` | not yet opened | no |
